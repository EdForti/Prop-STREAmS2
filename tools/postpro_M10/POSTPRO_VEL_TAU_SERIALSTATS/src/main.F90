...
! Read stat3d.bin
 ...
 ALLOCATE(w_sym(nv_stat_3d, nxs, nys, nzs))
 ...
 DO kproc = 0, nprocz-1
  ...
  OPEN(11,file='../stat3d0_'//chx//'_0000_'//chz//'.bin',form='unformatted', &
   ...
   IF (kproc < nprocz/2) THEN
     ...
     if (masterproc) print *, ksym_start,ksym_end
     w_sym(:,:,:,ksym_start:ksym_end) = w_stat_3d(1:nv_stat_3d,1:nxs,1:nys,1:nzserial)
!
!   DO l = 1, nv_stat_3d
!    IF (masterproc) WRITE(*,*) 'Reading var = ', l
!    IF (kproc < nprocz/2) THEN
!     ksym_start = kproc*nzserial + 1
!     ksym_end = ksym_start + nzserial - 1
!     DO i = 1, nxs
!       DO j = 1, nys
!         DO k = ksym_start, ksym_end
!           w_sym(l,i,j,k) = w_stat_3d(l,i,j,k-ksym_start+1)
!         END DO
!       END DO
!      END DO
!     END IF
  ...
  ELSE
    ...
    DO m = 1, nv_stat_3d
      ...
      IF ((m /= 4) .AND. (m /= 11) .AND. (m /= 12) ) THEN
        w_sym(m,:,:,ksym_start:ksym_end) = &
             & 0.5_rkind * (w_sym(m,:,:,ksym_start:ksym_end) + w_stat_3d(m, 1:nxs, 1:nys, nzserial:1:-1))
...
! Define output quantities
 ...
 DO j = 1, nysmax
   DO i = 1, nv_stat_3d
   CALL MPI_GATHER(w_sym(i,:,j,nzs), nxs, mpi_prec, w_slice_sym(i,:,j), nxs, &
