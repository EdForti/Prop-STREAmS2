...
!
 subroutine ddx(nx,u,x,iorder,dudx)
  implicit none
  integer, intent(in) :: nx, iorder
  real(rkind) , dimension(nx), intent(in) :: u,x
  real(rkind) , dimension(nx), intent(out):: dudx
  real(rkind) , dimension(3,3):: cc
  real(rkind) , dimension(3,7,3):: cl,cr
  integer :: lmax,i,l,ll
  real(rkind) :: du,dx,du_l,dx_l,du_r,dx_r
! 
  lmax = iorder/2
  if (lmax>3) then
...
!
  do i=1+lmax,nx-lmax
   ...
   dx = 0._rkind
   do l=1,lmax
    du = du + cc(l,lmax)*(u(i+l)-u(i-l))
    dx = dx + cc(l,lmax)*(x(i+l)-x(i-l))
   enddo
   dudx(i) = du/dx
  enddo
!
  do i=1,lmax
   ...
   do l=-lmax,lmax
    ll = 1+l+lmax 
    du_l = du_l + cl(i,ll,lmax)*u(ll)
    dx_l = dx_l + cl(i,ll,lmax)*x(ll)
    du_r = du_r - cl(i,ll,lmax)*u(nx-(l+lmax))
    dx_r = dx_r - cl(i,ll,lmax)*x(nx-(l+lmax))
   enddo
   dudx(i)      = du_l/dx_l
   dudx(nx-i+1) = du_r/dx_r
  enddo
