...
!
 ...
 integer :: i,j,m,lsp,lr,j99,ii,jint
 real(rkind), dimension(nx,ny) :: ufav,vfav,wfav,tfav,hfav,dufavdy
 real(rkind), dimension(nx) :: d99_vec,deltav_vec,utau_vec,rhow_vec,ttw_vec,dudyw_vec,rmuw_vec,gamw_vec,ppw_vec
 real(rkind), dimension(nx,ny,N_S) :: yyfav 
 real(rkind), dimension(nx,N_S) :: yyw_vec
...
!  write(10,*) 'zone i=',nx,', j=',ny
  do j=1,ny
   do i=1,nx
    ...
    do lsp=1,N_S
     yyfav(i,j,lsp) = wstat(i,j,86+lsp)/wstat(i,j,1)
    enddo
    hfav(i,j) = wstat(i,j,81)/wstat(i,j,1)
     write(10,100) x(i),y(j),wstat(i,j,1)!(wstat(i,j,m),m=1,nv)
   enddo
  enddo
!   close(10)
  do i=1,nx
...
!    Coefficients from Lele pag 39
     ...
     ppw   = aai*wstat(i,1,5 )+bbi*wstat(i,2,5 )+cci*wstat(i,3,5 )
     ttw   = aai*wstat(i,1,6 )+bbi*wstat(i,2,6 )+cci*wstat(i,3,6 )
     rmuw  = aai*wstat(i,1,20)+bbi*wstat(i,2,20)+cci*wstat(i,3,20)
     cpw   = aai*wstat(i,1,69)+bbi*wstat(i,2,69)+cci*wstat(i,3,69)
     gamw  = aai*wstat(i,1,70)+bbi*wstat(i,2,70)+cci*wstat(i,3,70)
     rlamw = aai*wstat(i,1,71)+bbi*wstat(i,2,71)+cci*wstat(i,3,71)
     tauw  = aai*wstat(i,1,44)+bbi*wstat(i,2,44)+cci*wstat(i,3,44)
!     tauw  = tauw+aai*wstat(i,1,46)+bbi*wstat(i,2,46)+cci*wstat(i,3,46)
!     tauw  = tauw+aai*wstat(i,1,47)+bbi*wstat(i,2,47)+cci*wstat(i,3,47)
     qw    = aai*wstat(i,1,76)+bbi*wstat(i,2,76)+cci*wstat(i,3,76)
!     qw    = qw+aai*wstat(i,1,75)+bbi*wstat(i,2,75)+cci*wstat(i,3,75)
!     qw    = qw+aai*wstat(i,1,77)+bbi*wstat(i,2,77)+cci*wstat(i,3,77)
     hhw   = aai*hfav(i,1)+bbi*hfav(i,2)+cci*hfav(i,3)
     rnuw  = rmuw/rhow
     do lsp=1,N_S
      yyw_vec(i,lsp) = aai*yyfav(i,1,lsp)+bbi*yyfav(i,2,lsp)+cci*yyfav(i,3,lsp)
     enddo
   else
...
!    rnuw  = rmuw/rhow
   ...
   rfac   = (rmuw*cpw/rlamw)**(1._rkind/3._rkind)
...
!   qw     = rlamw*dtdyw
   ...
   cf    = tauw/pdyn
   trec  = t0 * (1.+0.5*(gamw-1.)*rfac*Mach**2)
   Bq    = qw/(rhow*utau*hhw)
   if (theta_wall==1._rkind) then
    ch = 0._rkind
   else
    ch = qw/(2._rkind*pdyn*u0)
   endif
   j99   = 1
   do j=1,ny-1
    uu = ufav(i,j)
...
!
   ...
   mach_edge = wstat(i,jint,33) 
   !print *, mach_edge, wstat(33,i,jint)
   rmfac  = rfac*(gamw-1._rkind)*0.5_rkind*mach_edge**2
   ff     = ttw/tte
   !print *, ttw,tte,mach_edge,rfac
   aa     = sqrt(rmfac/ff)
   ...
   do ii=1, nstatloc
    if (i == ixstat(ii) .and. ii == 1) then
        write(*,'(A12,A15,A15,A15,A15,A15,A15,A15,A15,A15)') &
            'Station', 'redstar', 'retau', 'retheta', 'redelta2', &
            'dstar/delta99', 'theta/delta99', 'shapef', '-Bq*100', 'cf*1000'

        write(*,'(A147)') repeat('-', 147)  
    end if

    if (i == ixstat(ii)) then
        write(*,'(I12,F15.3,F15.3,F15.3,F15.3,F15.3,F15.3,F15.3,F15.3,F15.3)') &
            i, ue * rhoe * dstar / rmue, retau, retheta, redelta2, dstar / delta99, &
            theta / delta99, shapef, -Bq * 100, cf * 1000
    end if
   end do
!
   ...
   !                    9   10  11   12    13    14   15  16  17 
                      rhow,ttw,ppw,prmsp,deltav,utau,tauw,cf,cfinc, &
   !                      18      19       20      21  22 23 24          25 - 25+N_S-1
                      redelta99,retheta,redelta2,retau,Bq,ch,gamw,(yyw_vec(i,lsp), lsp=1,N_S)
  enddo
  close(10)
!
  do ii = 1,nstatloc
...
!  vaux(3,:) = vaux(1,:)*vaux(2,:)
   ...
   vaux(6,:) = utau/sqrt(gamw*ppw/rhow) ! Mach_tau constant
!   
   do j=1,ny
...
!    Mtau    = utau/sqrt(gam*ttw)
    Mtau    = sqrt(tauw/rhom)/sqrt(wstat(i,1,70)*ppm/rhom)
    rhormsp = (sqrt(wstat(i,j, 7)-wstat(i,j,1)**2))/rhom!/(rhow*gamw*Mtau**2)
    trmsp   = (sqrt(wstat(i,j,26)/wstat(i,j,1)-tfav(i,j)**2))/ttm!/(ttm*wstat(i,1,70)*Mtau**2)
    prmsp    = sqrt(abs(wstat(i,j,11)-wstat(i,j,5)**2))/tauw
    mum     = wstat(i,j,20)
    hrrp    = wstat(i,j,86+4*N_S+1)*mum/(rhom*tauw*hfav(i,j))
    ! note: ytp=ystar, yh=ystar, yvd=yp
    !              1     2    3         4        5   6   7
    write(15,100) y(j),rhom,ufav(i,j),vfav(i,j),ttm,ppm,mum, &
    !              8   9   10  11
                  y99,yp,ystar,yvp, &
    !             12  13   14  15   16  17
    ...
    !             23  24  25  26
                  u2p,v2p,w2p,uvp, &
    !             27       28    29   30       31 - 31+N_S-1               31+N_S - 31+2*N_S-1              31+2*N_S 
                  rhormsp,trmsp,prmsp,rhofac,(yyfav(i,j,lsp),lsp=1,N_S),(wstat(i,j,86+2*N_S+lsp),lsp=1,N_S),hrrp
   enddo
   close(15)
!
  enddo
