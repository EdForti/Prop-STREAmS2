...
contains
    ...
    subroutine read_input
        ...
        call flow_params_cfg%get("flow_params","delta0",l0)
        ...
        call flow_params_cfg%get("flow_params","theta_wall",theta_wall)
        call flow_params_cfg%get("flow_params","Reynolds",Reynolds) ! Bulk for channel, re delta for BL
        call flow_params_cfg%get("flow_params","nv_stat",nv) ! Bulk for channel, re delta for BL

        mixture = importphase('input_cantera.yaml')
        N_S = nspecies(mixture)
        N_R = nreactions(mixture)
        write(*,*) 'Number of species   N_S=',N_S
        write(*,*) 'Number of reactions N_R=',N_R
        ...
        call post_cfg%get("postpro","save_budget_plot3d",save_budget_plot3d)
    ...
    subroutine read_grid_bl
        implicit none
        integer :: i,j,l
        real(rkind), dimension(3,nx,ny,1) :: grid
        ...
        !write grid in p3d format
        do j=1,ny
         do i=1,nx
          grid(1,i,j,1) = x(i)
          grid(2,i,j,1) = y(j)
          grid(3,i,j,1) = 0._rkind
         enddo
        enddo
        open(10,file='POSTPRO/grid2d.xyz',form='unformatted',access='stream')
        write(10) nx,ny,1
        do l=1,3
         write(10) grid(l,1:nx,1:ny,1)
        enddo
        close(10)
    ...
    subroutine read_stat
        ...
        write(*,'(A,I3.3,A)') 'reading ',nv,' variables in stat'
        allocate(wstat(nx,ny,nv))

        if(recompute_avg > 0) then
            ...
            do it=it_start,it_end,it_out
                ...
                !open(11,file='AVZ/wavzplot_'//chstore//'.bin',form='unformatted')
                ...
                !do i=1,nx
                !do m=1,nv
                !    wstat(i,j,m) = wstat(i,j,m) + wstatz(i,j,m)
                !enddo
                !enddo
                !enddo
            enddo
