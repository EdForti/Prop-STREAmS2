...
x_e          = 31*D       # Activation point
Lx           = 19*D     # Length after activation point where deltax = deltax_ratio * deltax_e
alpha        = 0.3        # deltax increase from -inf to 0 of alpha*deltax_e
#deltax_e     = (1.3*D/NpD)*(1+alpha) # deltax at activation point
deltax_e     = 94e-6*3     # deltax at activation point
...
deltax_func  = lambda x: (deltax_e/(1+alpha) * (1 + alpha/np.log(2)*np.log(1+np.exp(b*(x-x_e))))) \
    + (deltax_e/(1+alpha) * (1 + alpha/np.log(2)*np.log(1+np.exp(b*(-x+x_e-12*D))))) - deltax_e;
...
plt.axvline(19*D, color='blue')
plt.axvline(25*D, color='red')
plt.axvline(31*D, color='blue')
plt.xlabel(r'$x$')
