...
x_e          = 21*D       # Activation point
Lx           = 60*D     # Length after activation point where deltax = deltax_ratio * deltax_e
alpha        = 0.20        # deltax increase from -inf to 0 of alpha*deltax_e
#deltax_e     = (1.3*D/NpD)*(1+alpha) # deltax at activation point
deltax_e     = 60e-6     # deltax at activation point
deltax_ratio = 10         # deltax after Lx from activation / deltax at activation
...
csi, x, nx, L = ggg.gengridgen(deltax_func, mode='N2L', nx=nx, edge1=0.9*L, edge2=1.2*L, write=True)
print('Final L = '+str(L))
