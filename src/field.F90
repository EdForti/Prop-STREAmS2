...
module streams_field_object
    ...
    contains
        ...
        procedure, pass(self) :: write_vtk_general
...
!
        integer, dimension(3) :: ncoords_nei
!
        associate(myrank => self%myrank, mp_cart => self%mp_cart, &
                  lxly => self%lxly, lxry => self%lxry, rxly => self%rxly, rxry => self%rxry, &
                  lxlz => self%lxlz, lxrz => self%lxrz, rxlz => self%rxlz, rxrz => self%rxrz, &
                  lylz => self%lylz, lyrz => self%lyrz, rylz => self%rylz, ryrz => self%ryrz, &
 ...
 1004 format(I4.4)
    ...
    subroutine write_plot3d(self,mach,reynolds,time,istore,w_io,plot3dgrid,plot3dfield,file_prefix,w_aux_io,l1,l2,l3,l4,l5)
     class(field_object), intent(inout) :: self
    ! Writing field (MPI I/O)
     integer, optional :: l1,l2,l3,l4,l5
     integer :: l1_,l2_,l3_,l4_,l5_
     ...
     l1_          = 1        ; if (present(l1))           l1_         = l1
     l2_          = 2        ; if (present(l2))           l2_         = l2
     l3_          = 3        ; if (present(l3))           l3_         = l3
     l4_          = 4        ; if (present(l4))           l4_         = l4
     l5_          = 6        ; if (present(l5))           l5_         = l5
!
...
!
         ...
         if (self%masterproc) then
          write(*,*) 'Storing grid in plot3D format'
...
!
         if (present(w_io)) then
             ...
             nv_io = 5
             allocate(w_io_(5,1:self%nx,1:self%ny,1:self%nz))
             w_io_(1,:,:,:) = w_aux_io(:,:,:,l1_)
             w_io_(2,:,:,:) = w_aux_io(:,:,:,l2_)
             w_io_(3,:,:,:) = w_aux_io(:,:,:,l3_)
             w_io_(4,:,:,:) = w_aux_io(:,:,:,l4_)
             w_io_(5,:,:,:) = w_aux_io(:,:,:,l5_)
             file_prefix_ = "flow_"
         endif
!
         if (self%masterproc) then
...
!
         ...
         if (present(w_io)) offset = 4*size_integer
         do l=1,nv_io
          call mpi_file_set_view(mpi_io_file,offset,mpi_prec,filetype,"native",mpi_info_null,self%mpi_err)
...
!
    ...
    subroutine write_vtk(N_S, species_names, self, time, istore, w_aux_io,l1,l2,l3,l4,l5,l6)
     class(field_object), intent(inout)           :: self            !< The field.
    ! Writing field (MPI I/O)
     real(rkind), optional :: time
     real(rkind) ::  time_
     integer, optional :: l1,l2,l3,l4,l5,l6
     integer :: l1_,l2_,l3_,l4_,l5_,l6_
     integer :: istore
     integer, intent(in) :: N_S
     character(len=30), dimension(N_S), intent(in) :: species_names
     character(32) :: file_prefix_
     real(rkind), dimension(:,:,:,:), allocatable :: w_io_
     real(rkind), dimension(:,:,:,:) :: w_aux_io
     integer :: nv_io
...
!    
     time_        = 1._rkind ; if (present(time))        time_       = time
     l1_          = 1+N_S    ; if (present(l1))          l1_         = l1
     l2_          = 2+N_S    ; if (present(l2))          l2_         = l2
     l3_          = 3+N_S    ; if (present(l3))          l3_         = l3
     l4_          = 4+N_S    ; if (present(l4))          l4_         = l4
     l5_          = 5+N_S    ; if (present(l5))          l5_         = l5
     l6_          = 6+N_S    ; if (present(l6))          l6_         = l6
!
...
!
     nv_io = 6+N_S
     file_prefix_ = "flow_"
     allocate(w_io_(6+N_S,1:self%nx,1:self%ny,1:self%nz),names(nv_io))
     do lsp=1,N_S
      w_io_(lsp,:,:,:) = w_aux_io(:,:,:,lsp)
     enddo
     w_io_(1+N_S,:,:,:) = w_aux_io(:,:,:,l1_)
     w_io_(2+N_S,:,:,:) = w_aux_io(:,:,:,l2_)
     w_io_(3+N_S,:,:,:) = w_aux_io(:,:,:,l3_)
     w_io_(4+N_S,:,:,:) = w_aux_io(:,:,:,l4_)
     w_io_(5+N_S,:,:,:) = w_aux_io(:,:,:,l5_)
     w_io_(6+N_S,:,:,:) = w_aux_io(:,:,:,l6_)
!     
     do lsp=1,N_S
      write(names(lsp), '(A,A)') "Y_", trim(species_names(lsp))       
     end do
     names(1+N_S) = "density"
     names(2+N_S) = "velocity_x"
     names(3+N_S) = "velocity_y"
     names(4+N_S) = "velocity_z"
     names(5+N_S) = "temperature"
     names(6+N_S) = "pressure"
!
     if (self%masterproc) then
      offset_x = 0 
...
!    
    ...
    subroutine write_vtk_general(self, it_vtr, file_prefix, namefolder, &
                    nimax, njmax, nkmax, ni, nj, nk, &
                    i1, i2, j1, j2, k1, k2, mp_cart, my_master, &
                    num_aux_slice, list_aux_slice, w_aux_io, aux_names)

    class(field_object), intent(inout)           :: self            !< The field.

     integer, parameter :: int64_kind = selected_int_kind(2*range(1))

     integer, intent(in) :: it_vtr, nimax, njmax, nkmax, ni, nj, nk, num_aux_slice
     integer, intent(in) :: i1, i2, j1, j2, k1, k2, mp_cart
     integer, dimension(:), intent(in) :: list_aux_slice
     real(rkind), dimension(1:,1:,1:,1:), intent(in) :: w_aux_io
     character(14), intent(in) :: file_prefix
     character(14), intent(in) :: namefolder
     logical, intent(in) :: my_master
     character(len=30), dimension(:), intent(in)  :: aux_names

     integer :: nv_io, l, mpi_io_file, filetype, size_real, ntot, ll
     ...
     integer, dimension(mpi_status_size) :: istatus
     !real(rkind), dimension(:,:,:,:), allocatable :: w_io_
     character(len=    6) :: nastore
     character(len=    7) :: vtk_float
     character(len=  100) :: namefile
     character(len=65536) :: xml_part

     write(nastore(1:6), '(I6.6)') it_vtr
     namefile = trim(namefolder)//'/'//trim(file_prefix)//'_'//nastore//'.vtr'
     ...
     kg2 = nkmax
     if (i1==i2) then
        ig1 = i1 +self%ncoords(1)*self%nx
        ig2 = ig1
     endif
     if (j1==j2) then
        jg1 = j1 +self%ncoords(2)*self%ny
        jg2 = jg1
     endif
     if (k1==k2) then
        kg1 = k1 +self%ncoords(3)*self%nz
        kg2 = kg1
     endif
     sizes(1)    = ig2-ig1+1 !self%nblocks(1)*ni
     sizes(2)    = jg2-jg1+1 !self%nblocks(2)*nj
     sizes(3)    = kg2-kg1+1 !self%nblocks(3)*nk
     subsizes(1) = i2-i1+1
     subsizes(2) = j2-j1+1
     subsizes(3) = k2-k1+1
     starts(1)   = 0 + self%ncoords(1)*subsizes(1)
     ...
     nv_io = num_aux_slice
     !allocate(w_io_(1:num_aux_slice,1:ni,1:nj,1:nk))
     !do l = 1, num_aux_slice
     !  w_io_(l,:,:,:) = w_aux_io(:,:,:,list_aux_slice(l))
     !enddo
  ...
  &   <PointData> '
       ...
       do l=1,nv_io
        ll = list_aux_slice(l)
        xml_part = trim(adjustl(xml_part)) // ' &
       & <DataArray type="'//vtk_float//'" NumberOfComponents="1" Name="'//trim(aux_names(ll))//'" format="appended" &
       &  offset="'//int2str_o(offset)//'"/>'
        offset = offset + delta_offset_w
       enddo
       xml_part = trim(adjustl(xml_part)) // ' &
       ...
       write(123) size_real*int(nimax,int64_kind) , self%grid%xg(ig1:ig2)
       write(123) size_real*int(njmax,int64_kind) , self%grid%yg(jg1:jg2)
       write(123) size_real*int(nkmax,int64_kind) , self%grid%zg(kg1:kg2)
       flush(123)
      close(123)
     endif
!
     ...
     do l=1,nv_io
      ll = list_aux_slice(l)
      call MPI_BARRIER(mp_cart,self%mpi_err)
      if (my_master) then
      ...
      call MPI_BARRIER(mp_cart, self%mpi_err)
      call MPI_FILE_OPEN(mp_cart,trim(namefile),MPI_MODE_RDWR,MPI_INFO_NULL,mpi_io_file,self%mpi_err)
      call MPI_FILE_GET_SIZE(mpi_io_file, offset, self%mpi_err)
      call MPI_BARRIER(mp_cart, self%mpi_err)
      call MPI_FILE_SET_VIEW(mpi_io_file,offset,mpi_prec,filetype,"native",MPI_INFO_NULL,self%mpi_err)
      call MPI_FILE_WRITE_ALL(mpi_io_file,w_aux_io(i1:i2,j1:j2,k1:k2,ll),ntot,mpi_prec,istatus,self%mpi_err)
     enddo
