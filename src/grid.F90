...
module streams_grid_object
    ...
    private
    public :: grid_object
    public :: GRID_FROMFILE, GRID_UNIFORM, GRID_CHA, GRID_BL, GRID_SBLI, GRID_2BL, GRID_BL_LAM
    ...
    integer(ikind), parameter :: GRID_2BL             = 6_ikind
    integer(ikind), parameter :: GRID_BL_LAM          = 7_ikind
    ...
    contains
        ...
        procedure, pass(self) :: generate_grid_geoprogression_2bl
        procedure, pass(self) :: generate_grid_uni_extension
        procedure, pass(self) :: generate_yg
        procedure, pass(self) :: generate_yg_2bl
...
!
        if (self%grid_type == GRID_FROMFILE) then
            ...
            select case (self%grid_type)
                ...
                case(GRID_CHA)
                    call fail_input_any('CHA grid not supported')
                    call self%generate_grid_cha(grid_vars(1), grid_vars(2),nint(grid_vars(3)), grid_vars(4))
                case(GRID_BL,GRID_SBLI,GRID_BL_LAM)
                    self%domain_size = self%domain_size/grid_vars(6) ! adim wrt ddelta0
                    call self%generate_grid_bl(grid_vars(1), grid_vars(2), nint(grid_vars(3)), grid_vars(4), grid_vars(7))
                    if (nint(grid_vars(3))<self%nymax) then
                     if (self%grid_type==GRID_BL .or. self%grid_type==GRID_BL_LAM) then
                      call self%generate_grid_geoprogression(nint(grid_vars(3)), grid_vars(4))
                     else
                      call fail_input_any('SBLI grid not supported')
                      call self%generate_grid_uni_extension(nint(grid_vars(3)))
                     endif
                    endif
                    call self%generate_yg(nint(grid_vars(5)),ystaggering)
                    self%domain_size = self%domain_size*grid_vars(6)
                    self%yg = self%yg*grid_vars(6)
                    self%yn = self%yn*grid_vars(6)
                case(GRID_2BL)
                    self%is_y_staggered = .true.
                    self%domain_size = self%domain_size/grid_vars(6) ! adim wrt ddelta0
                    call self%generate_grid_bl(grid_vars(1), grid_vars(2), nint(grid_vars(3)), grid_vars(4), grid_vars(7))
                    if (nint(grid_vars(3))<self%nymax/2) then
                      call self%generate_grid_geoprogression_2bl(nint(grid_vars(3)), grid_vars(4))
                    endif
                    call self%generate_yg_2bl(nint(grid_vars(5)),ystaggering)
                    self%domain_size = self%domain_size*grid_vars(6)
                    self%yg = self%yg*grid_vars(6)
                    self%yn = self%yn*grid_vars(6)
...
!
    subroutine generate_grid_geoprogression_2bl(self, nymaxwr, rlywr)
        class(grid_object), intent(inout) :: self  !< The grid.
        real(rkind),        intent(in) :: rlywr
        integer,            intent(in) :: nymaxwr
        real(rkind) :: dy1, rlygp, rgp, rgpold
...
!
      endassociate
!
    endsubroutine generate_grid_geoprogression_2bl

    subroutine generate_yg_2bl(self, nsmooy, ystaggering)
        class(grid_object), intent(inout) :: self  !< The grid.
        integer,            intent(in) :: nsmooy
        logical,            intent(in) :: ystaggering
        integer :: j, l, nymaxloc
        real(rkind), allocatable, dimension(:) :: tmpyn
!
        associate(nymax=>self%nymax, ng => self%ng, yg => self%yg, yn => self%yn, domain_size => self%domain_size)
!
        nymaxloc =nymax/2+1 
        allocate(tmpyn(nymaxloc))

        !SMOO FA CASINI, SISTEMA
        do l=1,nsmooy
         tmpyn = yn(1:nymaxloc)
         do j=2,nymaxloc-1
          yn(j) = tmpyn(j-1)+4._rkind*tmpyn(j)+tmpyn(j+1)
...
!       ghost nodes
        do j=1,ng
         yg(1-j) = -yg(j)
         yg(nymax+j) = yg(nymax)+(yg(nymax)-yg(nymax-j))
        enddo
!
        endassociate
    endsubroutine generate_yg_2bl
...
!
        ...
        if (file_exists) then
         open(18,file='domain_size.dat')
          read(18,*) self%domain_size(1:3)
         close(18)
        else
         self%domain_size(1) = xg(nxmax)-xg(1)
         self%domain_size(2) = yg(nymax)-yg(1)
         self%domain_size(3) = zg(nzmax)-zg(1)
        endif
!
        if (is_y_staggered) then
         open(10,file='yn.dat')
