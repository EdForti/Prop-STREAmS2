...
contains
    ...
    subroutine bcswap_corner_step_1b_cuf(nx, ny, nz, nv, ng, w_gpu, &
             wbuf_lxly_s_gpu, wbuf_lxry_s_gpu, wbuf_rxly_s_gpu, wbuf_rxry_s_gpu, &
             wbuf_lylz_s_gpu, wbuf_lyrz_s_gpu, wbuf_rylz_s_gpu, wbuf_ryrz_s_gpu, &
             wbuf_lxlylz_s_gpu, wbuf_lxlyrz_s_gpu, wbuf_lxrylz_s_gpu, wbuf_lxryrz_s_gpu, &
    ...
    subroutine bcswap_corner_step_3b_cuf(nx, ny, nz, nv, ng, w_gpu, &
           wbuf_lxly_r_gpu, wbuf_lxry_r_gpu, wbuf_rxly_r_gpu, wbuf_rxry_r_gpu, &
           wbuf_lylz_r_gpu, wbuf_lyrz_r_gpu, wbuf_rylz_r_gpu, wbuf_ryrz_r_gpu, &
           wbuf_lxlylz_r_gpu, wbuf_lxlyrz_r_gpu, wbuf_lxrylz_r_gpu, wbuf_lxryrz_r_gpu, &
           wbuf_rxlylz_r_gpu, wbuf_rxlyrz_r_gpu, wbuf_rxrylz_r_gpu, wbuf_rxryrz_r_gpu, &
           lxly, lxry, rxly, rxry, &
           lylz, lyrz, rylz, ryrz, &
           lxlylz, lxlyrz, lxrylz, lxryrz, &
           rxlylz, rxlyrz, rxrylz, rxryrz)
      integer :: nx, ny, nz, ng, nv
      real(rkind), device, dimension(1-ng:,1-ng:,1-ng:,1:) :: w_gpu
    ...
    subroutine bcswap_edges_corners(self, steps)
     ...
     associate(nx => self%nx, ny => self%ny, nz => self%nz, &
               ...
               ileftbottom  => self%field%ileftbottom, irightbottom => self%field%irightbottom, &
               ilefttop     => self%field%ilefttop,    irighttop => self%field%irighttop,       &
               pbc          => self%field%grid%is_xyz_periodic,                                 &
               mp_cart      => self%field%mp_cart, iermpi => self%mpi_err, nrank => self%field%myrank, &
               ...
               lxlylz => self%field%lxlylz, lxlyrz => self%field%lxlyrz, lxrylz => self%field%lxrylz, rxlylz => self%field%rxlylz,&
               lxryrz => self%field%lxryrz, rxlyrz => self%field%rxlyrz, rxrylz => self%field%rxrylz, rxryrz => self%field%rxryrz,&
               wbuf_lxly_s_gpu   => self%wbuf_lxly_s_gpu   , wbuf_lxry_s_gpu   => self%wbuf_lxry_s_gpu, &
               wbuf_rxly_s_gpu   => self%wbuf_rxly_s_gpu   , wbuf_rxry_s_gpu   => self%wbuf_rxry_s_gpu, &
     ...
     if(steps_(1)) then
      call bcswap_corner_step_1_cuf(nx, ny, nz, nv, ng, w_gpu, &
             wbuf1s_c_gpu, wbuf2s_c_gpu, wbuf3s_c_gpu, wbuf4s_c_gpu)
      if(pbc(2)) then
       !print*,'calling edge corners swap'
       call bcswap_corner_step_1b_cuf(nx, ny, nz, nv, ng, w_gpu, &
             wbuf_lxly_s_gpu, wbuf_lxry_s_gpu, wbuf_rxly_s_gpu, wbuf_rxry_s_gpu, &
             wbuf_lylz_s_gpu, wbuf_lyrz_s_gpu, wbuf_rylz_s_gpu, wbuf_ryrz_s_gpu, &
             wbuf_lxlylz_s_gpu, wbuf_lxlyrz_s_gpu, wbuf_lxrylz_s_gpu, wbuf_lxryrz_s_gpu, &
             wbuf_rxlylz_s_gpu, wbuf_rxlyrz_s_gpu, wbuf_rxrylz_s_gpu, wbuf_rxryrz_s_gpu)
     ...
     if(steps_(2)) then
      ...
      if(pbc(2)) then
       ! x-y and full-z exchanges
       indc = nv*ng*ng*nz
       if (lxly == nrank) then
           wbuf_rxry_r_gpu = wbuf_lxly_s_gpu
           wbuf_lxly_r_gpu = wbuf_rxry_s_gpu
       else
           call mpi_sendrecv(wbuf_lxly_s_gpu,indc,mpi_prec,lxly ,5, &
                             wbuf_rxry_r_gpu,indc,mpi_prec,rxry ,5,mp_cart,istatus,iermpi)
           call mpi_sendrecv(wbuf_rxry_s_gpu,indc,mpi_prec,rxry ,6, &
                             wbuf_lxly_r_gpu,indc,mpi_prec,lxly ,6,mp_cart,istatus,iermpi)
       endif
       if (lxry == nrank) then
           wbuf_rxly_r_gpu = wbuf_lxry_s_gpu
           wbuf_lxry_r_gpu = wbuf_rxly_s_gpu
       else
           call mpi_sendrecv(wbuf_lxry_s_gpu,indc,mpi_prec,lxry ,7, &
                             wbuf_rxly_r_gpu,indc,mpi_prec,rxly ,7,mp_cart,istatus,iermpi)
           call mpi_sendrecv(wbuf_rxly_s_gpu,indc,mpi_prec,rxly ,8, &
                             wbuf_lxry_r_gpu,indc,mpi_prec,lxry ,8,mp_cart,istatus,iermpi)
       endif
       ! y-z and full-x exchanges
       indc = nv*nx*ng*ng
       if (lylz == nrank) then
           wbuf_ryrz_r_gpu = wbuf_lylz_s_gpu
           wbuf_lylz_r_gpu = wbuf_ryrz_s_gpu
       else
           call mpi_sendrecv(wbuf_lylz_s_gpu,indc,mpi_prec,lylz ,9, &
                             wbuf_ryrz_r_gpu,indc,mpi_prec,ryrz ,9,mp_cart,istatus,iermpi)
           call mpi_sendrecv(wbuf_ryrz_s_gpu,indc,mpi_prec,ryrz ,10, &
                             wbuf_lylz_r_gpu,indc,mpi_prec,lylz ,10,mp_cart,istatus,iermpi)
       endif
       if (lyrz == nrank) then
           wbuf_rylz_r_gpu = wbuf_lyrz_s_gpu
           wbuf_lyrz_r_gpu = wbuf_rylz_s_gpu
       else
           call mpi_sendrecv(wbuf_lyrz_s_gpu,indc,mpi_prec,lyrz ,11, &
                             wbuf_rylz_r_gpu,indc,mpi_prec,rylz ,11,mp_cart,istatus,iermpi)
           call mpi_sendrecv(wbuf_rylz_s_gpu,indc,mpi_prec,rylz ,12, &
                             wbuf_lyrz_r_gpu,indc,mpi_prec,lyrz ,12,mp_cart,istatus,iermpi)
       endif
       ! x-y-z exchanges
       indc = nv*ng*ng*ng
       if (lxlylz == nrank) then
           ...
           call mpi_sendrecv(wbuf_lxlylz_s_gpu,indc,mpi_prec,lxlylz ,13, &
                             wbuf_rxryrz_r_gpu,indc,mpi_prec,rxryrz ,13,mp_cart,istatus,iermpi)
           call mpi_sendrecv(wbuf_rxryrz_s_gpu,indc,mpi_prec,rxryrz ,14, &
                             wbuf_lxlylz_r_gpu,indc,mpi_prec,lxlylz ,14,mp_cart,istatus,iermpi)
           call mpi_sendrecv(wbuf_lxlyrz_s_gpu,indc,mpi_prec,lxlyrz ,15, &
                             wbuf_rxrylz_r_gpu,indc,mpi_prec,rxrylz ,15,mp_cart,istatus,iermpi)
           call mpi_sendrecv(wbuf_rxrylz_s_gpu,indc,mpi_prec,rxrylz ,16, &
                             wbuf_lxlyrz_r_gpu,indc,mpi_prec,lxlyrz ,16,mp_cart,istatus,iermpi)
           call mpi_sendrecv(wbuf_rxlylz_s_gpu,indc,mpi_prec,rxlylz ,17, &
                             wbuf_lxryrz_r_gpu,indc,mpi_prec,lxryrz ,17,mp_cart,istatus,iermpi)
           call mpi_sendrecv(wbuf_lxryrz_s_gpu,indc,mpi_prec,lxryrz ,18, &
                             wbuf_rxlylz_r_gpu,indc,mpi_prec,rxlylz ,18,mp_cart,istatus,iermpi)
           call mpi_sendrecv(wbuf_rxlyrz_s_gpu,indc,mpi_prec,rxlyrz ,19, &
                             wbuf_lxrylz_r_gpu,indc,mpi_prec,lxrylz ,19,mp_cart,istatus,iermpi)
           call mpi_sendrecv(wbuf_lxrylz_s_gpu,indc,mpi_prec,lxrylz ,20, &
                             wbuf_rxlyrz_r_gpu,indc,mpi_prec,rxlyrz ,20,mp_cart,istatus,iermpi)
       endif
      endif
     endif
     if(steps_(3)) then
      call bcswap_corner_step_3_cuf(nx, ny, nz, nv, ng, w_gpu, &
          ...
          ileftbottom, ilefttop, irightbottom, irighttop)
      if(pbc(2)) then
       call bcswap_corner_step_3b_cuf(nx, ny, nz, nv, ng, w_gpu, &
           wbuf_lxly_r_gpu, wbuf_lxry_r_gpu, wbuf_rxly_r_gpu, wbuf_rxry_r_gpu, &
           wbuf_lylz_r_gpu, wbuf_lyrz_r_gpu, wbuf_rylz_r_gpu, wbuf_ryrz_r_gpu, &
           wbuf_lxlylz_r_gpu, wbuf_lxlyrz_r_gpu, wbuf_lxrylz_r_gpu, wbuf_lxryrz_r_gpu, &
           wbuf_rxlylz_r_gpu, wbuf_rxlyrz_r_gpu, wbuf_rxrylz_r_gpu, wbuf_rxryrz_r_gpu, &
    ...
    subroutine bcswap_edges_corners_var(self, w_swap, steps)
     ...
     if(steps_(1)) then
      ...
      if(pbc(2)) then
       !print*,'calling edge corners swap'
       call bcswap_corner_step_1b_cuf(nx, ny, nz, 1, ng, w_swap, &
             wbuf_lxly_s_gpu, wbuf_lxry_s_gpu, wbuf_rxly_s_gpu, wbuf_rxry_s_gpu, &
             wbuf_lylz_s_gpu, wbuf_lyrz_s_gpu, wbuf_rylz_s_gpu, wbuf_ryrz_s_gpu, &
             wbuf_lxlylz_s_gpu, wbuf_lxlyrz_s_gpu, wbuf_lxrylz_s_gpu, wbuf_lxryrz_s_gpu, &
             wbuf_rxlylz_s_gpu, wbuf_rxlyrz_s_gpu, wbuf_rxrylz_s_gpu, wbuf_rxryrz_s_gpu)
     ...
     if(steps_(3)) then
      call bcswap_corner_step_3_cuf(nx, ny, nz, 1, ng, w_swap, &
          ...
          ileftbottom, ilefttop, irightbottom, irighttop)
      if(pbc(2)) then
       call bcswap_corner_step_3b_cuf(nx, ny, nz, 1, ng, w_swap, &
           wbuf_lxly_r_gpu, wbuf_lxry_r_gpu, wbuf_rxly_r_gpu, wbuf_rxry_r_gpu, &
           wbuf_lylz_r_gpu, wbuf_lyrz_r_gpu, wbuf_rylz_r_gpu, wbuf_ryrz_r_gpu, &
           wbuf_lxlylz_r_gpu, wbuf_lxlyrz_r_gpu, wbuf_lxrylz_r_gpu, wbuf_lxryrz_r_gpu, &
           wbuf_rxlylz_r_gpu, wbuf_rxlyrz_r_gpu, wbuf_rxrylz_r_gpu, wbuf_rxryrz_r_gpu, &
